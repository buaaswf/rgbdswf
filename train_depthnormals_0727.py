...
def get_instance_masks(labels,instances):

    return instances_masks,instance_labels
def dataread():
    import h5py
    f = h5py.File('/data1/work_swf/nyu_aligned/nyu_depth_v2_labeled.mat','r')
    imagedata = f['images']
    imagedepths = f['depths']
    imagelabels = f['labels']
    imageinstance = f['instances']
    return [imagedata, imagedepths, imagedepths]
...
def traindepth(data = None):
    train_set_x, train_set_y, train_set_m=dataread()
    import theano.tensor as T

    #  train_set_x, train_set_y, train_set_m = datasets[0]
    #  valid_set_x, valid_set_y, valid_set_m = datasets[1]
    #  test_set_x, test_set_y, test_set_m = datasets[2]
    index = T.lscalar()
    x = T.matrix ('x')
    y = T.ivector ('y0')
    m = T.matrix('m0')
    ...
    #model = Sequential()
    machine = machine2model()
    print "======>>", machine
    print x.get_value().shape
    pred = machine.infer_depth_and_normals(x[:,:,:,1])
    cost = machine.define_depths_cost(pred, y[:,1], m)
    for u in machine:
